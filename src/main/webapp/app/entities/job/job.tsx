...
export class Job extends React.Component<IJobProps> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="job-heading">
          <Translate contentKey="abstraktApp.job.home.title">Jobs</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="abstraktApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {jobList.map((job, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <Translate contentKey={`abstraktApp.Week.${job.week}`} />
                  </td>
                  <td>{job.complete ? 'true' : 'false'}</td>
                  <td>{job.paid ? 'true' : 'false'}</td>
                  <td>{job.customer ? <Link to={`customer/${job.customer.id}`}>{job.customer.name}</Link> : ''}</td>
